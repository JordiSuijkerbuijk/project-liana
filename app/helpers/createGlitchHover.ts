...
export default function createGlitchHover(wrapper: HTMLElement) {
  ...
  const exemptLetterAmount =
    lettersArray.length <= 3 ? lettersArray.length - 1 : Math.ceil((lettersArray.length - 3) / 2);
  ...
  animateAbleLetters.forEach((letter, i) => {
    if (letter instanceof HTMLElement) {
      ...
      const possibleCharacters = `abcdefghijklmnopqrstuvwxyz1234567890!@#$%^&*-=+[]';/~`.split('');
      const letterIndex = possibleCharacters.findIndex((v) => v === originalLetter);
